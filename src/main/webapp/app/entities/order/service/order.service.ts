...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'pickUpDate' | 'repairDate' | 'deliveryDate'> & {
...
export class OrderService {
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...order,
      pickUpDate: order.pickUpDate?.toJSON() ?? null,
      repairDate: order.repairDate?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...restOrder,
      pickUpDate: restOrder.pickUpDate ? dayjs(restOrder.pickUpDate) : undefined,
      repairDate: restOrder.repairDate ? dayjs(restOrder.repairDate) : undefined,
